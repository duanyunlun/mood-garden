...
**v0.4.0 · 原型重构版** —— 按原型 v5 重做了视觉、信息架构与收集机制：手写体与新色板、一级 Tab 改为**记录 / 时光 / 花园 / 我的**、全屏记录入口与全屏花园、图鉴并入花瓣页、收集单位由种子改为**花瓣**（一天记满 3 件得 1 片，5 片开一朵花）；静态分析零问题，**213 个测试**全部通过。
...
| Tab1 记录首页 | ✅ 完成 | 全屏极简：日期、一句提问、两个记录入口、今日进度 |
| Tab2 时光轴 | ✅ 完成 | 周 / 月 / 年三视图 + 日期详情（含灰烬封条） |
| Tab3 花园 | ✅ 完成 | 花苞五片花瓣逐片点亮、花园实景、各花种花瓣、花之图鉴（已并入本页） |
...
记录 → 选心情标签（标签与花种一一对应）→ **一天记满 3 件收获 1 片花瓣** →
5 片花瓣拼成一个花苞 → **绽放成花，花随时间在花园中真实生长**。

花瓣的花种由**当天用得最多的标签**决定（原型 v5 的规则）。
...
│   ├── shell/                   # 四个一级 Tab 的容器
│   ├── record_home/             # Tab1 记录首页（原型 screen-home）
│   ├── petals/                  # Tab3 花园：花苞 + 花瓣 + 图鉴 + 全屏花园
│   ├── garden/                  # 花园画布与养分进度条
│   ├── timeline/                # Tab2 时光轴（周/月/年）+ 日期详情
│   ├── shell/                   # 四 Tab 主壳与 Tab 切换控制器
│   ├── codex/                   # 花之图鉴（分区组件已抽出，供花瓣页复用）
│   ├── profile/                 # Tab4 我的（主题换肤可用）
│   ├── record_happy/            # 记录开心事 + 落地反馈动画
...
| 7.1.2 | 落地反馈动画 | `lib/features/record_happy/widgets/seed_landing_overlay.dart` |
| 7.1.3 | 花苞五片花瓣逐片点亮 | `lib/features/petals/widgets/petal_bud.dart` |
...
| 开花阈值与养分数值 | 取自原型（3 件得 1 片花瓣 / 5 片开一朵花 / +10 / +15），集中定义在 `AppConstants` |
